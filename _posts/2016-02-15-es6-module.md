...
Of course in order for all of this to function, we need some tools to manage
...
It's seemsl like it happens either because `webpack` adds a `use strict;`
...
To build and install `foo` so that it can be imported by others, just run the
...
`app/scripts/module_name.js` there `module_name` is the name give in the
generator questionnaire. To change it to something else, you can either
re-run the generator or make the following changes:

* Rename `app/scripts/mymodule.js`
* Change occurences of `mymodule` in `app/index.html`
