...
var monthFilteredText = function(monthFilter) {
    ...
    return function(point) {
        ...
        total += point.cclim[monthFilter[i]-1][weatherTypeIndeces[currentWeatherType]];

        if (monthFilter.length === 0)
            monthFilter.length = 1; //avoid division by zero in the next step

        if (currentWeatherType == 'high C' || currentWeatherType == 'low C')
            return "Average " + currentWeatherType + ": " + Math.floor(total / monthFilter.length);
        else
            return "Total " + currentWeatherType + ": " + Math.floor(total);
    };
};
    ...
    function ready(error, us, capitals, climate) {
        ...
        function resetView() {
            ...
            .on('mouseover', function(d) {
                ...
                labelSelect.selectAll('.label-text-climate').text(monthFilteredText(myMonthSelectorChart.monthFilterPrev()));
